...
- Manifest is generated by `vite-plugin-pwa` from config in `vite.config.js` (standalone display, portrait orientation, dark theme).
- Icons at 192px and 512px in `public/`.
- Service worker is auto-generated by `vite-plugin-pwa` (`generateSW` mode, `autoUpdate`). Precaches all build assets for full offline support.
